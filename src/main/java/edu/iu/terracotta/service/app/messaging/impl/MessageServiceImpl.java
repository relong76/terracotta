...
public class MessageServiceImpl implements MessageService {
    ...
    @Override
    public Message duplicate(Message message, MessageGroup messageGroup) {
        return messageRepository.saveAndFlush(
            Message.builder()
                .configuration(messageConfigurationService.duplicate(message.getConfiguration()))
                .content(messageContentService.duplicate(message.getContent()))
    ...
    @Override
    public MessageDto copy(Message fromMessage, Message toMessage) {
        messageConfigurationService.copy(fromMessage.getConfiguration(), toMessage.getConfiguration());
        messageContentService.copy(fromMessage.getContent(), toMessage.getContent());

        return toDto(
            messageRepository.saveAndFlush(toMessage)
        );
    }
