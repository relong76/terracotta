...
public class MessageContentServiceImpl implements MessageContentService {
    ...
    @Override
    public MessageContentDto update(MessageContentDto messageContentDto, MessageContent messageContent) {
        return toDto(
    ...
    @Override
    public MessageContent duplicate(MessageContent messageContent) {
        ...
        messageContent.getAttachments().forEach(
            messageContentAttachment -> messageContentAttachmentService.duplicate(messageContentAttachment, newMessageContent)
        );
    ...
    @Override
    public MessageContent copy(MessageContent fromContent, MessageContent toContent) {
        toContent.getAttachments()
            .forEach(
                attachment -> {
                    try {
                        messageContentAttachmentService.delete(attachment, toContent.getMessage().getGroup().getOwner());
                    } catch (CanvasApiException e) {
                        log.error("Error deleting attachment ID: [{}] from Canvas", attachment.getId(), e);
                    }
                }
            );

        toContent.getCustomPlaceholders()
            .forEach(
                customPlaceholder -> messageContentCustomPlaceholderService.delete(toContent.getMessage().getUuid(), toContent.getUuid(), customPlaceholder.getUuid())
            );

        toContent.getRules()
            .forEach(
                rule -> messageRuleService.delete(toContent.getMessage(), toContent.getUuid(), rule.getUuid())
            );

        toContent.setBody(fromContent.getBody());

        fromContent.getAttachments()
            .forEach(
                attachment -> messageContentAttachmentService.duplicate(attachment, toContent)
            );

        fromContent.getCustomPlaceholders()
            .forEach(
                customPlaceholder -> messageContentCustomPlaceholderService.duplicate(customPlaceholder, toContent)
            );

        fromContent.getRules()
            .forEach(
                rule -> messageRuleService.duplicate(rule, toContent)
            );

        return messageContentRepository.saveAndFlush(toContent);
    }
