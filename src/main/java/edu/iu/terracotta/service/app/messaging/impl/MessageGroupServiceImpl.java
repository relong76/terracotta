...
public class MessageGroupServiceImpl implements MessageGroupService {
    ...
    public MessageGroupDto update(MessageGroupDto messageGroupDto, MessageGroup messageGroup) {
        messageGroup = messageGroupRepository.save(
            fromDto(messageGroupDto, messageGroup)
        );
    ...
    public MessageGroupDto send(MessageGroup messageGroup) {
        messageGroup.setStatus(MessageStatus.QUEUED);
    ...
    public MessageGroupDto delete(MessageGroup messageGroup) {
        messageGroup.setStatus(MessageStatus.DELETED);
        messageGroup = messageGroupRepository.save(messageGroup);
    ...
    public MessageGroupDto duplicate(MessageGroup messageGroup, Exposure exposure, SecuredInfo securedInfo) {
        MessageGroup newMessageGroup = messageGroupRepository.saveAndFlush(
            MessageGroup.builder()
                ...
                .title(String.format("Copy of %s", messageGroup.getTitle()))
                .toConsentedOnly(messageGroup.isToConsentedOnly())
                .build()
        );

        messageGroup.getMessages().stream()
            .forEach(message -> messageService.duplicate(message, newMessageGroup));

        return toDto(messageGroupRepository.findById(newMessageGroup.getId()).get());
    }
    ...
    public MessageGroupDto toDto(MessageGroup messageGroup) {
        ...
        return MessageGroupDto.builder()
            ...
            .status(messageGroup.getStatus())
            .title(messageGroup.getTitle())
            .toConsentedOnly(messageGroup.isToConsentedOnly())
            .build();
    }

    @Override
    public MessageGroup fromDto(MessageGroupDto messageGroupDto, MessageGroup messageGroup) {
        if (messageGroup == null) {
            messageGroup = MessageGroup.builder().build();
        }

        messageGroup.setTitle(messageGroupDto.getTitle());
        messageGroup.setStatus(messageGroupDto.getStatus());
        messageGroup.setToConsentedOnly(messageGroupDto.isToConsentedOnly());
