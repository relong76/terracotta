...
public class IntegrationTokenServiceImpl implements IntegrationTokenService {
    ...
    @Override
    public void create(Submission submission, boolean isPreview) {
    ...
    @Override
    public IntegrationToken redeemToken(String launchToken) throws DataServiceException, IntegrationTokenNotFoundException, IntegrationTokenInvalidException {
        ...
        if (!integrationToken.isValid()) {
            invalidate(integrationToken);
            throw new IntegrationTokenInvalidException(String.format("Integration token: [%s] is not valid.", launchToken));
        }
