...
public class MessageController {
    ...
    public ResponseEntity<MessageDto> put(@PathVariable long experimentId, @PathVariable long exposureId, @PathVariable UUID groupUuid, @PathVariable UUID uuid, @RequestBody MessageDto messageDto, HttpServletRequest req) {
        ...
        try {
            ...
            message = apijwtService.messagingAllowed(securedInfo, uuid, groupUuid);
    ...
    public ResponseEntity<MessageDto> copy(@PathVariable long experimentId, @PathVariable long exposureId, @PathVariable UUID groupUuid, @PathVariable UUID uuid, @RequestBody MessageDto fromMessageDto, HttpServletRequest req) {
        SecuredInfo securedInfo = apijwtService.extractValues(req, false);
        Message fromMessage;
        Message toMessage;
        ...
        try {
            ...
            apijwtService.messagingGroupAllowed(securedInfo, exposureId, uuid);
            fromMessage = apijwtService.messagingAllowed(securedInfo, fromMessageDto.getGroupId(), fromMessageDto.getId());
            toMessage = apijwtService.messagingAllowed(securedInfo, groupUuid, uuid);
            apijwtService.experimentAllowed(securedInfo, experimentId);
            apijwtService.exposureAllowed(securedInfo, experimentId, exposureId);
        } catch (BadTokenException | ExperimentNotMatchingException | MessageOwnerNotMatchingException | MessageNotMatchingException | MessageGroupOwnerNotMatchingException | MessageGroupNotMatchingException | ExposureNotMatchingException | MessageGroupNotFoundException | MessageNotFoundException e) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        ...
        try {
            return new ResponseEntity<>(messageService.copy(fromMessage, toMessage), HttpStatus.OK);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
