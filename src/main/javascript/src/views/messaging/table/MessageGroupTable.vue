...
export default {
  ...
  computed: {
    ...
    displayedGroups() {
      ...
      return this.messageGroups[this.exposureSetIndex]?.filter((group) => group.status !== this.status.uneditable.deleted) || [];
    },
    groups: {
      get() {
        return this.messageGroups[this.exposureSetIndex] || [];
      },
      set(newGroup) {
        ...
        if (!this.messageGroups[this.exposureSetIndex]) {
          this.messageGroups[this.exposureSetIndex] = newGroup;
        } else {
          this.messageGroups[this.exposureSetIndex].push(...newGroup);
        }
      }
    },
    displayedGroupsCount() {
      return this.displayedGroups.length;
    },
    showTable() {
      return this.isLoaded && this.displayedGroupsCount > 0;
    },
  ...
  methods: {
    ...
    async handleGroupDuplicate(group) {
      try {
        ...
        this.messageGroupsExpanded[this.exposureSetIndex].splice(this.messageGroupsExpanded[this.exposureSetIndex].length, 0, response.data);
      } catch (error) {
        console.error("handleMoveGroup | catch", { error });
      }
    },
  ...
  async mounted() {
    ...
    if (!this.messageGroupsExpanded[this.exposureSetIndex]) {
      this.messageGroupsExpanded.splice(this.exposureSetIndex, 1, []);
    }
